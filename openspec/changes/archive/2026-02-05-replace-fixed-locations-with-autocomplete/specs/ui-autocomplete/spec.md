# Spec: UI Autocomplete Component
...
Returned results MUST be presented in a clear, clickable dropdown list to allow user selection.
#### Scenario: Displaying Results
- **Given** the API returns a list of locations
- **When** the dropdown renders
- **Then** each item should show the location `name`
- **And** optionally show the `address` if available
