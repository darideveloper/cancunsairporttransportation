...
### Styling
- **Fonts**: Use `font-title` (Metropolis) for navigation links and titles; `font-sans` (Inter Variable) for body text in mobile menu.
- **Colors**: 
  - Nav Links: Use `text-accent` (#894700) for active/link states.
  - Text: Use `text-black` for primary text and `text-gray-dark` for headings.
  - Background: White or `bg-gray-light` (#f9fafb) for subtle backgrounds.
  - CTA Button: Use `bg-accent` (or appropriate button variant) as the primary brand color.
  - Borders/Dividers: Use `border-gray` or `border-gray-light` where appropriate.
